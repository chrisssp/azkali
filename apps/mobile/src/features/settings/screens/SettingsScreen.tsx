    ...
    <Pressable onPress={onPress} android_ripple={{ color: '#f0f0f0' }}>
      <HStack className="items-center px-6 py-4" space="md">
        <Box className="w-8 h-8 items-center justify-center">
          <Ionicons name={icon} size={22} color={danger ? '#DC2626' : '#006341'} />
        </Box>
        <Text
...
function SectionLabel({ text }: { text: string }) {
  return (
    <Text className="px-6 pt-5 pb-2 text-xs font-semibold text-typography-400 uppercase tracking-widest">
      {text}
    </Text>
  );
}
...
export function SettingsScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      <Box className="bg-white rounded-2xl mx-4 border border-outline-100 overflow-hidden">
        <SettingsItem
          icon="person-outline"
          label="Cambiar datos"
          onPress={() => {}}
      ...
      <Box className="bg-white rounded-2xl mx-4 border border-red-100 overflow-hidden">
        <SettingsItem
