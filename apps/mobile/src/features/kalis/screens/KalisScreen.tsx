...
export const KalisScreen: React.FC = () => {
  ...
  return (
    <ScreenWrapper
      header={<GlobalHeader mode="settings" />}
      className="flex-1 bg-background-50"
    >
      <VStack className="flex-1">
        {/* Kali balance card */}
        <Box className="mb-4 bg-primary-900 rounded-2xl px-5 py-4">
          ...
          <Text className="text-xs text-primary-300 mt-2">
            Se generan al pagar con tu tarjeta Banco Azteca en el ecosistema
            Grupo Salinas
          </Text>
        </Box>

        {/* Section label */}
        <Text className="text-xs font-bold text-typography-400 uppercase tracking-widest mb-2">
          Historial de kalis
        </Text>

        {/* List */}
        <ScrollView className="flex-1">
          <VStack className="pb-8" space="sm">
            {items.length > 0 ? (
              items.map((item) => (
              ...
              <Box className="py-12 items-center">
                <Text className="text-center text-typography-400">
                  Aún no tienes kalis acumulados. ¡Empieza a pagar con tu
                  tarjeta Banco Azteca!
                </Text>
              </Box>
            )}
          </VStack>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} size="lg">
        ...
        >
          <ModalHeader className="pt-5 pb-3">
          ...
          <ModalBody className="px-2" style={{ flexShrink: 1 }}>
            <ScrollView
            ...
            >
              ...
              <Box className="bg-primary-900 rounded-t-xl px-3 py-2.5">
                <HStack>
                  <Text className="text-xs font-bold text-white flex-1">
                    Tipo de tarjeta
                  </Text>
                  <Text className="text-xs font-bold text-white w-20 text-right">
                    por $100
                  </Text>
                  <Text className="text-xs font-bold text-white w-20 text-right">
                    por $1,000
              ...
              {CONVERSION_RATES.map((rate, index) => (
                ...
                >
                  <HStack className="items-center">
                    <Text className="text-sm font-semibold text-primary-900 flex-1">
                      {rate.label}
                    </Text>
                    <Text className="text-sm text-warning-700 font-bold w-20 text-right">
                      {rate.per100} K
                    </Text>
                    <Text className="text-sm text-warning-700 font-bold w-20 text-right">
                      {rate.per1000} K
              ...
              <VStack space="sm">
                {TERMS.map((term, i) => (
                  <HStack key={i} space="xs" className="items-start">
                    ...
                    <Text className="text-xs text-typography-400 flex-1 leading-5">
                      {term}
                    </Text>
                  </HStack>
                ))}
              ...
              <Box className="mt-4 mb-2 bg-warning-50 rounded-xl p-3 border border-warning-200">
                <Text className="text-xs text-warning-800 leading-5 text-center">
                  Programa regulado por Banco Azteca S.A. Institución de Banca
                  Múltiple. CNBV Autorización No. 0248. Los kalis no constituyen
                  un depósito bancario ni instrumento de inversión.
                </Text>
              </Box>
            </ScrollView>
          </ModalBody>
